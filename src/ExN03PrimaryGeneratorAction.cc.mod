...
{    
  ...
  xsecFile = new TFile("Inputs/xsec_K.root","READ");
...
//cout<<"Details  pv1_LAB[2] "<<pv1_LAB[2]<<"  pv2_LAB[2] "<<pv2_LAB[2]<<" qpar "<<qpar<<" qper "<<qper<<endl;
  ...
  cout<<kinenergy1_LAB<<" "<< kinenergy2_LAB<<" "<< kinenergy3_LAB<<endl;
   //end of 3 body
    //particle gen
  cout<<"shoot part "<<endl;
    G4double x0 = G4RandGauss::shoot(MeanX, SigmaX);
    G4double y0 = G4RandGauss::shoot(MeanY, SigmaY);
    //G4double x0 = 0.;
    //G4double y0 = 0.;
    G4double z0 = CLHEP::RandFlat::shoot(150);
    particleGun->SetParticlePosition(G4ThreeVector(x0*mm,y0*mm,z0*mm));  
    //particleGun->SetParticlePosition(G4ThreeVector(0,0,0));  
    particleGun->SetParticleDefinition(G4KaonPlus::Definition());
    particleGun->SetParticleMomentumDirection(G4ThreeVector(pv1_LAB[0],pv1_LAB[1],pv1_LAB[2]));
    //particleGun->SetParticleMomentumDirection(G4ThreeVector(0,0,1));
    particleGun->SetParticleEnergy(kinenergy1_LAB*MeV);
    ...
    particleGun->SetParticleMomentumDirection(G4ThreeVector(pv2_LAB[0],pv2_LAB[1],pv2_LAB[2]));
    particleGun->SetParticleEnergy(kinenergy2_LAB*MeV);
    particleGun->GeneratePrimaryVertex(anEvent);
    cout<<"proton"<<endl;

    particleGun->SetParticlePosition(G4ThreeVector(x0*mm,y0*mm,z0*mm));  
    G4ParticleTable* particleTable = G4ParticleTable::GetParticleTable();  
    //G4ParticleDefinition *hypern = G4IonTable::GetIonTable()->FindIon(Z,A,0.);
    G4ParticleDefinition *hypern = particleTable->GetIon(Z,A,0.);
    particleGun->SetParticleDefinition(hypern);  
    particleGun->SetParticleMomentumDirection(G4ThreeVector(pv3_LAB[0],pv3_LAB[1],pv3_LAB[2]));
    particleGun->SetParticleEnergy(kinenergy3_LAB*MeV);
    particleGun->GeneratePrimaryVertex(anEvent);
    cout<<"HN"<<endl;
  ...
  //G4double x0 = 0.;
  //G4double y0 = 0.;
  G4double z0 = G4RandUniform::shoot();
  particleGun->SetParticlePosition(G4ThreeVector(x0*mm,y0*mm,z0*mm));  
